...
8 КЛАССОВ (все чинятся, кроме фундаментальных):
1. ДИСТРАКТОРЫ: каждый правдоподобен и ОДНОЗНАЧНО неверен; не синоним, не off-topic,
   не «формально-тоже-верный»; той же эпохи/домена. Если притянуты — СНАЧАЛА смени угол
   так, чтобы дистракторы стали чистыми. НИКОГДА не заменяй дистрактор синонимом/
   перефразом правильного ответа — это самоброй. drop="distractors" ТОЛЬКО если после
   смены угла всё равно нельзя дать 4 чистых дистрактора.
...
по своду (8 классов + ЯИ-запреты). Стратегия: FIX-FIRST.
