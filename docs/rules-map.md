...
- **Каноничный код:** `src/content-rules.js` — `validateOptionsHomogeneous` (около-спам, жёсткий),
  `validateNumericRanges` (вложенные «более N»/«менее N», жёсткий), `validateNumericTell`
  (верный — единственное не-круглое число, мягкое; берёт `correctAnswerIndex`), `validateEnumeration`
  (запятая→«и», мягкое). Проверяют 4 варианта ВМЕСТЕ, а не один ответ.
...
- Семантические правила (кодом не отловить, только промпт+свод): редкий термин в вопросе
  поясняй; величины в одной единице; дистрактор правдоподобен в контексте; **без бинарных
...
- Зависимые точки (обновлять вместе):
  ...
  - `scripts/polish.workflow.js` — класс 1(г) (диапазоны) + класс 9 (около-спам, «и»; дрейф-фикс: «и» не значимое слово).
